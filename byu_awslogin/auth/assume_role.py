...
def assume_role(account_role, samlAssertion, sessionDuration):
    conn = boto3.client('sts', config=client.Config(
...
def __get_all_account_roles(roles_by_account, role_name):
    if not role_name:
        print("{}You must specify a role using the {}--role{} option when assuming a role accross all accounts{}".format(
    ...
    for account_name in roles_by_account:
        if role_name in roles_by_account[account_name]:
            ...
            print("{}Will not try to assume role '{}{}{}' in account '{}{}{}' because you don't have access to that role{}".format(
                Colors.lyellow, Colors.cyan, role_name, Colors.lyellow, Colors.yellow, account_name, Colors.lyellow, Colors.normal))
...
def __get_single_account_role(roles_by_account, account_name, role_name):
    ...
    if not role_name or not roles_by_account[account_name][role_name]:
        role_names = __get_role_names_for_account(
...
def __prompt_for_account(account_names):
    ...
    while True:
        account = prompt("Account: ", completer=account_completer, complete_while_typing=True, validator=validator)
        if account == 'list':
            for account_name in account_names:
                if account_name != 'list':
                    print("\t{}{}{}".format(Colors.yellow,
                                            account_name, Colors.normal))
        else:
            return account
...
def __prompt_for_role(account_name, role_names):
    ...
    print('#   {}You have access to the following roles in {}{}{}   #'.format(
        Colors.white, Colors.yellow, account_name, Colors.lblue))
    print('#   {}Which role would you like to assume?{}{}         #'.format(
    ...
    return prompt("Select role: ", completer=role_completer, complete_while_typing=True, validator=validator)

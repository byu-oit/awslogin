...
def test_cached_login(
    mock_cache_adfs_auth,
    mock_write_to_config_file,
    mock_write_to_cred_file,
    mock_assume_role,
    mock_prompt_roles,
    mock_get_saml_assertion,
):
...
def test_non_cached_login(
    ...
    mock_authenticate.return_value = AdfsAuthResult(
        "FakeUrl", "FakeContext", "FakeResponse", "FakeSession"
    )
    mock_get_saml_assertion.return_value = SAMLAssertion("FakeAssertion")
    mock_prompt_roles.return_value = [
        AccountRole("FakeAccount1", "FakeRole1", "FakeArn1", "FakePrincipalArn"),

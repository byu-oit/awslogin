...
def test_ask_which_role_to_assume_single_role(role_cache):
    ...
    principal_roles = [
        [
            "arn:aws:iam::333333333333:saml-provider/ADFS",
        ...
        [
            "arn:aws:iam::222222222222:saml-provider/ADFS",
    ...
    account_roles = ask_which_role_to_assume(
        account_names, principal_roles, account_name, role_name
    )
    assert len(account_roles) == 1
    assert account_roles[0].account_name == "my-dev-account"
    assert account_roles[0].role_name == "ReadOnly"
    assert (
        account_roles[0].principal_arn == "arn:aws:iam::333333333333:saml-provider/ADFS"
    )
